...
{
    /// <summary>
    /// This class handles phone interface in calls app overview state
    /// </summary>
    public class PhoneInterface_AppOverview_Calls_State : PhoneInterface_CardAppOverviewBase_State
    {
        private struct CallCard
        {
            public CallCardHandler cardHandler;
            public PhoneInterface_Contacts_BaseData cardData;
        }
        
        private PhoneInterface_App_Contacts_Database database_;

        private List<CallCard> cards_ = new List<CallCard>();
        private CallCard activeCard_;

        public PhoneInterface_AppOverview_Calls_State(PhoneInterface_CardAppOverview_InitializerData initializerData,
        ...
        protected override void InitializeCards()
        {
            for (int i = 0; i < database_.database.Count; i++)
            {
                ...
                obj.TryGetComponent(out CallCardHandler handler);
                cards_.Add(new CallCard() {cardHandler = handler, cardData = data});
        ...
        protected override void HandleCardSelection()
        {
            ...
            var newStateData = new StateMachine_StateSwitchData()
                {nextState = typeof(PhoneInterface_CardOverview_Calls_State), nextStateDatabase = data};
