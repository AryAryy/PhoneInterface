...
namespace PhoneInterfaceCode.Extensions
{
    /// <summary>
    ...
    public class CoroutineManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Runs the passed function after the given delay in scaled time
        /// </summary>
        ...
        public Coroutine MoveObject(Transform targetObject, Vector3 targetPos, float duration, bool local)
        {
            var c = StartCoroutine(Routine());
            return c;
            ...
            {
                var timer = 0f;
                var startPos = local ? targetObject.localPosition : targetObject.position;
                while (timer < duration)
                {
                    var newPos = Vector3.Lerp(startPos, targetPos, timer / duration);
                    if (local) targetObject.localPosition = newPos;
                    else targetObject.position = newPos;
